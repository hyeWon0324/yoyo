...
async function uploadMusic(req: RequestWithFiles, res: Response): Promise<Response> {
  ...
  try{
    const ret: any = await new Promise(((resolve, reject) => {
      axios.post('http://localhost:7989/upload', formData, {
            headers:{
                  ...formHeaders,
              }
          })
...
async function uploadProfile(req: RequestWithFiles, res: Response): Promise<Response> {
  ...
  formData.append('profile_img', req.files.profile_img[0].buffer, {filename: req.files.profile_img[0].originalname});

  try{
    const ret: any = await new Promise(((resolve, reject) => {
      axios.post('http://localhost:7989/profile', formData, {
        headers:{
          ...formHeaders,
        }
      })
...
async function uplaodBg(req: RequestWithFiles, res: Response): Promise<Response> {
  const formData = new FormData();
  const formHeaders = formData.getHeaders();
  ...
  try{
    const ret: any = await new Promise(((resolve, reject) => {
      axios.post('http://localhost:7989/profile/bg', formData, {
        headers:{
          ...formHeaders,
        }
      })
